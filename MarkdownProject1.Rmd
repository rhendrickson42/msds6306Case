...
```
## Compute the median Alcohol Beverage Content (ABV) and international bitterness unit (IBU) for each state.  Plot a bar chart to compare.
```{r, ECHO=TRUE}
...
```{r, ECHO=TRUE}
StateSelected <- c('SD') #Copy and paste desited state inbetween the quotes here from list of States by alphabetical order.
...
ggplot(data=State_IBU, aes(x=reorder(State, -IBU), y=IBU)) + geom_bar(stat = "identity", fill=ColorOfPlotIBU) +  theme(axis.text.x=element_text(angle=90,vjust=0.5)) + xlab("State") + ylab("IBU") + ggtitle("IBU Median by State")
